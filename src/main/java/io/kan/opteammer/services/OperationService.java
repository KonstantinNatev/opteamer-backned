...
import io.kan.opteammer.mapper.OperationMapper;
import io.kan.opteammer.model.*;
import io.kan.opteammer.repositories.*;
...
public class OperationService {
    ...
    public OperationDTO createOperation(OperationDTO operationDTO) throws Exception {
        ...
        //validation  ------------
        boolean isValidOpTypeOperationProvider = new HashSet<>(operation.getTeamMembers().stream()
                .map(teamMember -> teamMember.getOperationProvider().getType().toString()).collect(Collectors.toList())).containsAll(
